...
exports.lastUrl  = function lastUrl() {
    var location = require('int_emp/cartridge/scripts/util/URLUtils').url('Home-Show');
    var click = session.clickStream.last;
    if (click) {
        location = require('int_emp/cartridge/scripts/util/URLUtils').url(click.pipelineName);
...
exports.lastCatalogURL = function lastCatalogURL() {
    ...
    var URLUtils = require('int_emp/cartridge/scripts/util/URLUtils');
    var locale = request.locale;
    request.setLocale(locale);
    var homeLocation = URLUtils.home();

    if (homeLocation) {
        return homeLocation;
    } else {
        return URLUtils.httpsHome().toString();
    }
};
